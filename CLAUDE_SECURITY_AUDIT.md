...
- B4-01 (NEW, Medium, Verified design gap): broker /start sets NO broker-owned cookie; /callback trusts only the signed broker_state and exchanges the attacker-suppliable `code` (server.ts /start 156-199 no res.cookie; /callback 201-292 no cookie check — CONFIRMED from my read). => authorization-code injection IF a live, unconsumed broker_state leaks (it's the `state` query param in the victim's browser at the provider) AND victim is lured to a crafted callback while logged in AND attacker holds an unconsumed provider code for their own account. The Showrunner nonce does NOT stop it because the attack reuses the victim's own broker_state (embeds victim nonce N). Impact = payment redirection / merchant-of-record swap for a site. Mitigated partly by Referrer-Policy:no-referrer (blocks referrer leak to provider), single-use state, 10-min TTL. Fix: broker sets httpOnly Secure SameSite cookie=hash(stateId) at /start, require match at /callback before code exchange. => AOC-02.
