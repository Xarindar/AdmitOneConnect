...
4. The broker exchanges `code` with the platform secret, signs a 5-minute `handoff` token with the client's shared secret, and redirects to the original Showrunner callback.
5. Showrunner verifies the handoff and stores the merchant access/refresh tokens encrypted in its own database.
...
- `returnOrigin`: the exact origin allowed for callbacks, with no path, query, or hash.
...
{"client_id":"client_1","refreshToken":"..."}
```
